...
function selectPatterns(patterns) {
    (0, helpers_1.it)(`should select patterns ${patterns.join(", ")}`, async function () {
        ...
        for (const pattern of patterns)
            await softwareSelection.processPattern_Final(pattern);
        console.log(`All patterns are successfully selected.`);
...
class SoftwareSelectionPage {
    ...
    async processPattern_inProgress(patternName) {
        ...
        const checkbox = await this.page.$(`input[type=checkbox][aria-labelledby*=${patternName}-title]`);
        if (!checkbox) {
            throw new Error(`Could not find checkbox for pattern: ${patternName}`);
        }
        // Check current state
        const isCurrentlyChecked = await checkbox.evaluate((cb) => cb.checked);
        if (isCurrentlyChecked) {
            console.log(`Pattern ${patternName} is already selected`);
            return { patternName, status: "already-selected" };
        }
